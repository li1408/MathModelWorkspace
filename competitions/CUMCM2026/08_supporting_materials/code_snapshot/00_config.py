...
def get_logger(name: str) -> logging.Logger:
    ...
    file_handler = logging.FileHandler(logs_dir / f"{name}_{timestamp()}.txt", encoding="utf-8")
...
def list_raw_data_files() -> list[Path]:
    ...
    return sorted(path for path in raw_dir.rglob("*") if path.is_file() and path.name not in ignored)
